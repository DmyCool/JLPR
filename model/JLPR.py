...
def do_task(crime_label, embs, epoch, display=True):
    ...
    with open('./output/8. newtest_crime_price/pred_truth_e{}.txt'.format(epoch), 'w') as f:
...
def write_emb_assign(epoch, ncluster, emb, assign):
    ffolder = './output/8. newtest_crime_price/m_{}/'.format(ncluster)
...
def write_metrics(epoch, ncluster, mae, rmse, r2, mape, s_num):
    fname = './output/8. newtest_crime_price/metrics_m_{}.txt'.format(ncluster)
    f = open(fname, 'a+')
...
feat = np.concatenate((poi, img), axis=1)
