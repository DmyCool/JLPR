...
import numpy as np
import pandas as pd
from createTract import create_tract
...
def process_trip():
    ...
    trip = trip[['Pickup Census Tract', 'Dropoff Census Tract', 'Pickup Community Area','Dropoff Community Area',
                 'Pickup Centroid Latitude', 'Pickup Centroid Longitude', 'Dropoff Centroid Latitude', 'Dropoff Centroid Longitude']] # 共22334743条记录
...
def create_flow_mtx():
    ...
    trip['count'] = 0
    OD = trip.groupby(['Pickup Census Tract', 'Dropoff Census Tract'])['count'].count()  # OD.index:
...
# process POI data
# '''['Restaurant', 'Hotel', 'Shopping mall', 'Scenic spot', 'Entertainment & Sport', 'Education', 'Medical service',
# 'Transportation facility', 'Real estate','Government & Organization', 'Cultural media', 'Life service', 'Financial Service', 'Other']'''
def poi_features():
    # 53296 records，14 categories
    ...
    if len(poi_mtx) != len(tractid_number):
        missing_tract = [index for index in tractid_number if index not in list(poi_mtx.index)]  # 在tractid_number中却不在poi_mtx中的tract
...
def get_img():
    dir_path = 'Z:\\dataresource\\GoogleStreetView\\Chicago\\Panorama\\'
    sample_index = pd.read_csv('./data/image/image_sample_index.csv')
    sample_id = list(sample_index['imgID'])

    # copy image into folder respectively
...
def get_image_features(filepath, indexfile):
    ...
    for i in range(len(tractid_numbers)):
        ...
        temp = train_img.loc[train_img['geoid10'] == tractid_numbers[i]]  # 在train_img中提取当前tract行
        imgid = list(temp['imgID'])  # obtain image index
        ...
        mean_feat = np.mean(np.array(feat_list), axis=0)
    ...
    if not os.path.exists('./data/img_feat_mtx.npy'):
        np.save('./data/img_feat_mtx.npy', img_features)  # img_feat_mtx shape: ()
...
def get_crime_count(train, test):
    ...
    train_crime_count = train_crime_count.sort_index()  # (801,)
    test_crime_count = test_crime_count.sort_index()  # (801,)

    print(train_crime_count.shape, test_crime_count.shape)

    if not os.path.exists('./data/crime_train.npy'):
        np.save('../data/crime_train.npy', train_crime_count)
        np.save('../data/crime_test.npy', test_crime_count)
